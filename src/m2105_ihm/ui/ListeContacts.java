...
public class ListeContacts extends javax.swing.JPanel  {
    ...
    public void setSelectedContact(Contact contact) {
        ...
        if (! listeNoeudContacts.isLeaf()) {
           for (int i=0; i<listeNoeudContacts.getChildCount(); i++) {
                node = (DefaultMutableTreeNode) listeNoeudContacts.getChildAt(i);
                data = (NodeData) node.getUserObject();
                if (data.id.equals(contact)) {
    ...
    public void setSelectedGroupe(GroupeContacts groupe) {
        NodeData data;       
        DefaultMutableTreeNode node;
        
        if (! listeNoeudGroupes.isLeaf()) {
           for (int i=0; i<listeNoeudGroupes.getChildCount(); i++) {
                node = (DefaultMutableTreeNode) listeNoeudGroupes.getChildAt(i);
                data = (NodeData) node.getUserObject();
                if (data.id.equals(groupe)) {
                    highlightSelected(node);
                }
           }
        }
